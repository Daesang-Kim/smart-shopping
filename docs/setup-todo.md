...
- cron(`src/app/api/cron/sync/route.ts` → `syncAllActiveItems`)은 고정 목록이 아니라 **그동안 실제로 조회돼서 DB에 쌓인 품목들만** 갱신함 — 사용량에 따라 자연스럽게 커버리지가 늘어남. 단, 한 번에 4개씩(`last_synced_at` 오래된 순)만 처리하고 6시간마다 돌아서 여러 번에 걸쳐 순환 갱신함 (아래 트레이드오프 참고)
...
- **cron 안정성 개선 (실측 기반)**:
  ...
  - 근본 해결로 **cron이 한 번에 4개씩만(`items.last_synced_at`이 가장 오래된 순), 6시간마다 순환 갱신**하도록 변경 (`syncAllActiveItems`) — 품목이 아무리 늘어도 1회 실행시간은 항상 배치 크기만큼으로 유지됨
...
- **cron이 4개씩만 순환 갱신**하므로, 활성 품목이 많이 늘어나면 개별 품목의 갱신 주기가 하루 이상으로 벌어질 수 있음 (현재 규모에선 문제없음, 필요시 배치 크기/주기 조정)
- Vercel Hobby 플랜의 cron 실행 빈도 제한에 따라 `vercel.json`의 6시간 주기가 실제로는 더 드물게 실행될 수 있음 (대시보드에서 실제 실행 이력 확인 권장)
